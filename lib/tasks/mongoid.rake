  ...
  namespace :db do
    unless Rake::Task.task_defined?("db:drop")
      ...
      task :drop => :environment do
        Mongoid::Clients.default.database.collections.each {|col| col.drop_indexes && col.drop unless ['system.indexes', 'system.users'].include?(col.name) }
      end
    end
    ...
    unless Rake::Task.task_defined?("db:seed")
      ...
      task :seed => :environment do
        seed_file = File.join(Rails.root.to_s, 'db', 'seeds.rb')
